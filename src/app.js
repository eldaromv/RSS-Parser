...
const generateId = (post, index) => (post.id ? post.id.replace(/\D/g, '') : index.toString());

const processPosts = (posts) => posts.map((post, index) => ({
  ...post,
  id: generateId(post, index),
}));
  ...
  const getFeedRequest = (url) => {
    axiosInstance.get(getRssData(url))
      .then(({ data }) => {
        ...
        watchedState.posts = [...watchedState.posts, ...processedPosts];
  ...
  const updatePosts = () => {
    ...
    const promises = feeds.map(({ url }) => axiosInstance.get(getRssData(url))
      .then((response) => {
        ...
        const processedNewPosts = processPosts(newPosts);
        watchedState.posts.push(...processedNewPosts);
      })
      .catch((error) => error));
